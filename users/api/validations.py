    ...
    def is_valid(self, bundle, request=None):
        ...
        if bundle.data.get('username'):
            try:
                ...
                errors['username'] = [
                    _(u'Nombre de usuario: Debe ser un correo electrónico.')]
        else:
            errors['username'] = [
                _(u'Nombre de usuario: Este campo es requerido.')]

        # First name required and longer than 2 characters
        if bundle.data.get('first_name'):
            if len(bundle.data['first_name']) < 3:
                errors['first_name'] = [
                    _(u'Nombre(s): Este campo debe tener al menos 3 caracteres.')]
        else:
        ...
        if bundle.data.get('last_name'):
            if len(bundle.data['last_name']) < 3:
                errors['last_name'] = [
                    _(u'Apellido(s): Este campo debe tener al menos 3 caracteres.')]
        else:
        ...
        if bundle.data.get('password'):
            if len(bundle.data['password']) < 5:
                errors['password'] = [
                    _(u'Contraseña: Este campo debe tener al menos 5 caracteres.')]
        else:
